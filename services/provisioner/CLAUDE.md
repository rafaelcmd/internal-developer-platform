...
```
cmd/consumer/main.go   - entry point: SQS poll/process loop
...
- **Traces:** `internal/telemetry` builds an OTLP/gRPC TracerProvider and installs
  W3C propagators. AWS SDK calls are instrumented with `otelaws` middleware
  (replaces `xray.Client`); the poll loop and per-message processing get explicit
  spans (`PollSQSMessages`, `ProcessMessage`, `GetSQSQueueURL`).
